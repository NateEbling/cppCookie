...
namespace Cookie {
	...
	Window* Window::create(const char* title)
	{
		...
		if (!monitor)
		{
			std::cout << "Failed to get primary monitor" << std::endl; // Make this go to a log
			return nullptr;
		}
		...
		if (!mode)
		{
			std::cout << "Failed to get video mode of primary monitor" << std::endl; // Make this go to a log
			return nullptr;
		}

		// Smallest monitor size is 300x200
		...
		glfwGetWindowSize((GLFWwindow*)res->windowPtr, &windowHeight, &windowHeight);

		glfwSetWindowPos((GLFWwindow*)res->windowPtr, monitorX + (mode->width - windowWidth) / 2, monitorY + (mode->height - windowHeight) / 2);
